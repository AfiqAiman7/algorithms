...
{ 
    ...
    void deleteNode(Object temp)
    {
        // Store first node
        Node ptr = first;
        Node prev = null;        // If first node itself holds the key to be deleted
        if (ptr != null && ptr.data == temp)
        {
            first= first.next; // Changed first
            return;
        }
        // Search for the key to be deleted, keep track of the
        // previous node as we need to change ptr.next
        while (ptr != null && ptr.data != temp)
        {
            prev = ptr;
            ptr = ptr.next;
        }
